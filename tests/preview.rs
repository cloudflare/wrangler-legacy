...
fn it_previews_with_config_text() {
    ...
    wrangler_toml.config = Some(config);
    fixture.create_wrangler_toml(wrangler_toml);
...
#[test]
fn it_previews_with_env_config_text() {
    let fixture = Fixture::new();
    fixture.create_file(
        "index.js",
        r#"
    ...
    env.config = Some(env_vars);

    let mut wrangler_toml = WranglerToml::with_env("test-preview-with-env-config", env);
    let mut vars: HashMap<&'static str, &'static str> = HashMap::new();
    vars.insert("HELLO", hello);
    vars.insert("WORLD", "overwrite this");
    wrangler_toml.config = Some(vars);

    fixture.create_wrangler_toml(wrangler_toml);

    preview_succeeds_with(&fixture, Some(TEST_ENV_NAME), "hello world");
}
